...
%s
  ...
  local function _decode_proto(di)
    ...
    for _ci = 1, nk do
      ...
      elseif t == 3 then
        consts[#consts + 1] = _d[di] == 1; di = di + 1
      elseif t == 4 then
        local sub, new_di = _decode_proto(di)
        consts[#consts + 1] = sub
        di = new_di + 1
      end
    end
    ...
    local code = {}
    for _ii = 1, nc do
      local op = _d[di]; di = di + 1
      local a = _d[di]; di = di + 1
      local b = _d[di]; di = di + 1
      local c = _d[di]; di = di + 1
      code[_ii] = {op=op, a=a, b=b, c=c}
    end
  ...
  local function _exec_proto(proto, regs, base, nargs, upvals, varargs)
    ...
    H[OP_MOVE] = function() regs[base + A] = regs[base + B] end
    H[OP_GETGLOBAL] = function() regs[base + A] = _G[consts[B]] end
    H[OP_SETGLOBAL] = function() _G[consts[B]] = regs[base + A] end
    H[OP_GETUPVAL] = function()
      if upvals and upvals[B + 1] then
        local uv = upvals[B + 1]
        regs[base + A] = uv[1][uv[2]]
    ...
    H[OP_NEWTABLE] = function() regs[base + A] = {} end
    H[OP_ADD] = function()
      local l, r = regs[base + B], regs[base + C]
      regs[base + A] = (type(l) == "number" and type(r) == "number") and (l + r) or (tonumber(l) or 0) + (tonumber(r) or 0)
    end
    H[OP_SUB] = function() regs[base + A] = regs[base + B] - regs[base + C] end
    H[OP_MUL] = function() regs[base + A] = regs[base + B] * regs[base + C] end
    H[OP_DIV] = function() regs[base + A] = regs[base + B] / regs[base + C] end
    H[OP_IDIV] = function() regs[base + A] = regs[base + B] // regs[base + C] end
    H[OP_MOD] = function() regs[base + A] = regs[base + B] %% regs[base + C] end
    H[OP_POW] = function() regs[base + A] = regs[base + B] ^ regs[base + C] end
    H[OP_BAND] = function() regs[base + A] = regs[base + B] & regs[base + C] end
    H[OP_BOR] = function() regs[base + A] = regs[base + B] | regs[base + C] end
    H[OP_BXOR] = function() regs[base + A] = regs[base + B] ~ regs[base + C] end
    H[OP_SHL] = function() regs[base + A] = regs[base + B] << regs[base + C] end
    H[OP_SHR] = function() regs[base + A] = regs[base + B] >> regs[base + C] end
    H[OP_UNM] = function() regs[base + A] = -regs[base + B] end
    H[OP_BNOT] = function() regs[base + A] = ~regs[base + B] end
    H[OP_NOT] = function() regs[base + A] = not regs[base + B] end
    H[OP_LEN] = function() regs[base + A] = #regs[base + B] end
    H[OP_CONCAT] = function()
      local v = regs[base + B]
      local s = type(v) == "string" and v or tostring(v)
      for j = B + 1, C do
    ...
    H[OP_FORLOOP] = function()
      ...
      local idx = regs[base + A]
      local cont = ((step > 0) and (idx <= limit)) or ((step <= 0) and (idx >= limit))
      if cont then regs[base + A + 3] = idx; pc = pc + sBx end
    end
    H[OP_TFORLOOP] = function()
      local sBx = B
      if sBx > 0x7FFFFFFF then sBx = sBx - 0x100000000 end
    ...
    H[OP_TFORPREP] = function()
      ...
      if sBx > 0x7FFFFFFF then sBx = sBx - 0x100000000 end
      local fn = regs[base + A]
      local a1 = regs[base + A + 1]
      local a2 = regs[base + A + 2]
      local r1, r2, r3 = fn(a1, a2)
      regs[base + A + 3] = r1
      regs[base + A + 4] = r2
      regs[base + A + 5] = r3
      pc = pc + sBx
    ...
    H[OP_TFORCALL] = function()
      ...
      local fn = regs[base + A]
      local r = {}
      for i = 1, n do r[i] = regs[base + A + i] end
      local res = {fn(table.unpack(r))}
      for i = 1, n do regs[base + A + 2 + i] = res[i] end
    end
    H[OP_SETLIST] = function()
      local t = regs[base + A]
      if not t then t = {}; regs[base + A] = t end
      if C == 0 then
        local n = regs[base + A + 1]
        for i = 1, n do t[#t + 1] = regs[base + A + 1 + i] end
      else
        for i = 1, C do t[#t + 1] = regs[base + A + i] end
      end
    end
    H[OP_CLOSURE] = function()
      local sub_proto = consts[B]
      if type(sub_proto) == "table" and sub_proto.code then
        ...
        regs[base + A] = function(...)
          ...
          local args = {...}
          local n = select("#", ...)
          for ai = 1, sub_proto.numparams do sub_regs[ai - 1] = args[ai] end
          local vargs = {}
          for vi = sub_proto.numparams + 1, n do vargs[#vargs + 1] = args[vi] end
          return _exec_proto(sub_proto, sub_regs, 0, n, captured_upvals, vargs)
        end
      end
    end
    H[OP_VARARG] = function()
      if B == 0 then
        if varargs then
          for j = 1, #varargs do regs[base + A + j - 1] = varargs[j] end
          for j = #varargs + 1, proto.maxstack - 1 do regs[base + A + j - 1] = nil end
        end
      else
        for j = 1, B - 1 do
          regs[base + A + j - 1] = varargs and varargs[j] or nil
        end
      end
    end
    H[OP_RETURN] = function()
    ...
    H[OP_CALL] = function()
      ...
      if type(fn) == "function" then
        ...
        if C > 0 then
          for j = 1, C - 1 do regs[base + A + j - 1] = results[j] end
        else
          regs[base + A] = results[1]
        end
      end
  ...
  return function(...)
    ...
    local _vargs = {}
    for _ai = 1, _np do
      _regs[_ai - 1] = _args[_ai]
    end
    for _vi = _np + 1, _nargs do
      _vargs[#_vargs + 1] = _args[_vi]
    end
    return _exec_proto(_main_proto, _regs, 0, _nargs, nil, _hv and _vargs or nil)
  end
end
end)()
]=====]
...
protect_as_expr(source_code, opts)
  ...
  再生成可调用的 VM chunk（每个函数独立 blob），最后返回替换代码。
...
local function protect_as_expr(source_code, opts)
  ...
  for idx, func_stmt in ipairs(func_stmts) do
    ...
    local named_params = {}
    for i, param in ipairs(func_stmt.params) do
      if param ~= "..." then
        named_params[#named_params + 1] = param
        sub.var_regs[param] = i - 1
        sub.reg_count = i
    ...
    for _, s in ipairs(func_stmt.body.stmts) do
      sub:compile_stmt(s)
    end
    sub:resolve_gotos()
    if #sub.code == 0 or sub.code[#sub.code].op ~= OPC.RETURN then
    ...
    local proto = {
      ...
      has_vararg = (#func_stmt.params > 0 and func_stmt.params[#func_stmt.params] == "..."),
    }
    local key = random_int(1, 0xFFFFFF)
    local vm_chunk = generate_function_vm(proto, key)
    local stub_name = "__vm_" .. tostring(idx)
  ...
  local function emit_expr(e)
    ...
    if e.type == "number" then return tostring(e.value)
    elseif e.type == "string" then
      local s = e.value:gsub("\\", "\\\\"):gsub('"', '\\"'):gsub("\n", "\\n"):gsub("\t", "\\t")
      return '"' .. s .. '"'
    elseif e.type == "bool" then return e.value and "true" or "false"
    elseif e.type == "nil" then return "nil"
    elseif e.type == "ident" then return e.name
    elseif e.type == "vararg" then return "..."
    elseif e.type == "binop" then
      return "(" .. emit_expr(e.left) .. " " .. e.op .. " " .. emit_expr(e.right) .. ")"
    elseif e.type == "unop" then
      return e.op .. emit_expr(e.expr)
    elseif e.type == "call" then
      local args = {}
      for _, a in ipairs(e.args or {}) do args[#args + 1] = emit_expr(a) end
      return emit_expr(e.func) .. "(" .. table.concat(args, ", ") .. ")"
    elseif e.type == "table" then
      local items = {}
      for _, f in ipairs(e.fields or {}) do
        if f.key then
          items[#items + 1] = "[" .. emit_expr(f.key) .. "] = " .. emit_expr(f.value)
        else
          items[#items + 1] = emit_expr(f.value)
        end
      end
      return "{ " .. table.concat(items, ", ") .. " }"
    elseif e.type == "index" then
      return emit_expr(e.obj) .. "[" .. emit_expr(e.key) .. "]"
    elseif e.type == "func_def" then
      local args = {}
      for _, a in ipairs(e.params or {}) do args[#args + 1] = a end
  ...
  local function emit_stmt(s, indent)
    ...
    if s.type == "assign" then
      local lv = {}
      for _, v in ipairs(s.targets) do lv[#lv + 1] = emit_expr(v) end
      local rv = {}
      for _, v in ipairs(s.values) do rv[#rv + 1] = emit_expr(v) end
      return indent .. table.concat(lv, ", ") .. " = " .. table.concat(rv, ", ")
    elseif s.type == "local" then
      local names = {}
      for _, n in ipairs(s.names or {}) do names[#names + 1] = n end
      local vals = {}
      for _, v in ipairs(s.values or {}) do vals[#vals + 1] = emit_expr(v) end
      if #vals == 0 then
        return indent .. "local " .. table.concat(names, ", ")
      else
        return indent .. "local " .. table.concat(names, ", ") .. " = " .. table.concat(vals, ", ")
      end
    elseif s.type == "call" then
      return indent .. emit_expr(s) .. ";"
    ...
    elseif s.type == "if" then
      local lines = {}
      lines[#lines + 1] = indent .. "if " .. emit_expr(s.cond) .. " then"
      for _, st in ipairs(s.then_block or {}) do lines[#lines + 1] = emit_stmt(st, indent .. "  ") end
      if s.else_block then
        lines[#lines + 1] = indent .. "else"
        for _, st in ipairs(s.else_block) do lines[#lines + 1] = emit_stmt(st, indent .. "  ") end
      end
      lines[#lines + 1] = indent .. "end"
      return table.concat(lines, "\n")
    elseif s.type == "while" then
      local lines = {}
      lines[#lines + 1] = indent .. "while " .. emit_expr(s.cond) .. " do"
      for _, st in ipairs(s.body or {}) do lines[#lines + 1] = emit_stmt(st, indent .. "  ") end
      lines[#lines + 1] = indent .. "end"
      return table.concat(lines, "\n")
    elseif s.type == "for_num" then
      local step = s.step and emit_expr(s.step) or "1"
      return indent .. "for " .. s.name .. " = " .. emit_expr(s.init) .. ", " .. emit_expr(s.limit) .. ", " .. step .. " do"
    elseif s.type == "do_block" then
      local lines = {indent .. "do"}
      for _, st in ipairs(s.body or {}) do lines[#lines + 1] = emit_stmt(st, indent .. "  ") end
      lines[#lines + 1] = indent .. "end"
      return table.concat(lines, "\n")
    elseif s.type == "break" then
      return indent .. "break"
    elseif s.type == "for_in" then
      local names = {}
      for _, n in ipairs(s.names or {}) do names[#names + 1] = n end
      local iter_str = emit_expr(s.iter_expr)
      return indent .. "for " .. table.concat(names, ", ") .. " in " .. iter_str .. " do"
    else
  ...
  for _, func_stmt in ipairs(func_stmts) do
    ...
    local params_str = table.concat(func_stmt.params, ", ")
    out_lines[#out_lines + 1] = "local function " .. name .. "(" .. params_str .. ")"
...
function M.set_config(options)
  ...
  elseif has_pass_key then
    -- Disable all pass keys first, then enable only the ones specified
    for _, key in ipairs(PASS_KEYS) do
      Config[key] = false
    end
    for key, value in pairs(options) do
  ...
  -- Sync config changes to PassManager
  sync_config_to_passes()
end
